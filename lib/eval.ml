...
let eval_binop binop e1 e2 = match binop, e1, e2 with
...
let rec step (expr, s_table) = match expr with
  | Int _ | Bool _ -> failwith "Does not step eval"
  | Var x -> (match STable.find_opt x s_table with
    | Some e when is_val e -> e,  s_table
    | Some e -> let (e', s_table') = step (e, s_table) in e', STable.add x e s_table'
    | None -> failwith "Unbound variable.")
  | Binop (op, e1, e2) when is_val e1 && is_val e2 -> eval_binop op e1 e2, s_table
